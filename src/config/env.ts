...
export const env = envSchema.validateSync({
  ...
  AWS_ACCESS_KEY_ID: Buffer.from(process.env.AWS_ACCESS_KEY_ID ?? '', 'base64').toString('utf-8'),
  AWS_SECRET_ACCESS_KEY: Buffer.from(process.env.AWS_SECRET_ACCESS_KEY ?? '', 'base64').toString('utf-8'),
  AWS_SESSION_TOKEN: Buffer.from(process.env.AWS_SESSION_TOKEN ?? '', 'base64').toString('utf-8'),
});
